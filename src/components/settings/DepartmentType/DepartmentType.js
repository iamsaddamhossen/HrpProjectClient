...
import { createDepartmentType, updateDepartmentType, getDepartmentTypes, deleteDepartmentType } from "../../../actions/departmentTypeAction";
...
const DepartmentType = () => {
    ...
    const saveDepartmentType = async () => {
        ...
        debugger;
        if (department.departmentTypeName.trim()) {

            if (department.departmentTypeId) {
                await dispatch(updateDepartmentType(department.departmentTypeId, department))
                    .then(response => {
                ...
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Department Updated!', life: 1500 });

            }
            else {

                await dispatch(createDepartmentType(department.departmentTypeName)).then((department) => {
                    setDepartment({
                ...
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Department Created!', life: 1500 });
    ...
    const deleteDepartment = async () => {
        ...
        let _departments = department.departmentTypeId;

        await dispatch(deleteDepartmentType(_departments))
            .then(response => {
                console.log(response);
            })
            .catch(e => {
    ...
    const onInputChange = (e, departmentTypeName) => {
        const val = (e.target && e.target.value) || '';
        let _department = { ...department };
        _department[`${departmentTypeName}`] = val;
    ...
    const header = (
        <div className="table-header">
            <h5 className="p-m-0">List of Department Types</h5>
    ...
    const departmentDialogFooter = (
        <React.Fragment>
            ...
            <button class="p-button p-component p-button-success" onClick={saveDepartmentType}>
    ...
    return (
        <div className="datatable-crud-demo">
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={departmentTypes} selection={selectedDepartments} onSelectionChange={(e) => setSelectedDepartments(e.value)}
                    dataKey="departmentTypeId" paginator rows={10} rowsPerPageOptions={[5, 10, 25]}
                    ...
                    <Column field="departmentTypeName" header="Name" sortable></Column>
            ...
            <Dialog visible={departmentDialog} style={{ width: '450px' }} header="Department Type" modal className="p-fluid" footer={departmentDialogFooter} onHide={hideDialog}>
                <div className="p-field">
                    <label htmlFor="departmentTypeName">Department Name <span style={{ color: 'red' }}>*</span></label>
                    <InputText id="departmentTypeName" value={department.departmentTypeName} onChange={(e) => onInputChange(e, 'departmentTypeName')} required autoFocus className={classNames({ 'p-invalid': submitted && !department.departmentTypeName })} />
                    {submitted && !department.departmentTypeName && <small className="p-error">Department Name is required.</small>}
                </div>
            </Dialog>
            ...
            <Dialog visible={deleteDepartmentDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteDepartmentDialogFooter} onHide={hideDeleteDepartmentDialog}>
                <div className="confirmation-content">
                    ...
                    {department && <span>Are you sure you want to delete <b>{department.departmentTypeName}</b>?</span>}
                </div>
            </Dialog>
